...
cp "$ROOT_DIR/install-memory-core-user.sh" "$BUNDLE_DIR/install-memory-core-user.sh"
cp "$ROOT_DIR/scripts/package-memory-core.sh" "$BUNDLE_DIR/scripts/package-memory-core.sh"
cp "$ROOT_DIR/scripts/memory-core-user.sh" "$BUNDLE_DIR/scripts/memory-core-user.sh"
...
chmod +x "$BUNDLE_DIR/install-memory-core.sh" "$BUNDLE_DIR/install-memory-core-user.sh" "$BUNDLE_DIR/scripts/package-memory-core.sh" "$BUNDLE_DIR/scripts/memory-core-user.sh"

printf '{\n' >"$MANIFEST_PATH"
printf '  "bundle_name": %s,\n' "\"$BUNDLE_NAME\"" >>"$MANIFEST_PATH"
printf '  "product": "memory-core",\n' >>"$MANIFEST_PATH"
printf '  "bundle_version": "5",\n' >>"$MANIFEST_PATH"
printf '  "installer": "install-memory-core.sh",\n' >>"$MANIFEST_PATH"
printf '  "user_installer": "install-memory-core-user.sh",\n' >>"$MANIFEST_PATH"
printf '  "builder": "scripts/package-memory-core.sh",\n' >>"$MANIFEST_PATH"
printf '  "default_install_mode": "repo-local-v4",\n' >>"$MANIFEST_PATH"
printf '  "included_assets": [\n' >>"$MANIFEST_PATH"
printf '    "AGENTS.md",\n' >>"$MANIFEST_PATH"
printf '    ".project-memory",\n' >>"$MANIFEST_PATH"
printf '    ".automation",\n' >>"$MANIFEST_PATH"
printf '    ".claude/agents",\n' >>"$MANIFEST_PATH"
printf '    "install-memory-core-user.sh",\n' >>"$MANIFEST_PATH"
printf '    "scripts/memory-core-user.sh"\n' >>"$MANIFEST_PATH"
printf '  ],\n' >>"$MANIFEST_PATH"
printf '  "install_modes": [\n' >>"$MANIFEST_PATH"
printf '    "repo-local-v4",\n' >>"$MANIFEST_PATH"
printf '    "user-level-v5"\n' >>"$MANIFEST_PATH"
printf '  ],\n' >>"$MANIFEST_PATH"
